...
const platColors: Record<Platform, string> = {
  ...
  YouTube:   "bg-rose-500/15 text-rose-400 border-rose-500/30",
  Douyin:    "bg-cyan-500/15 text-cyan-400 border-cyan-500/30",
  Instagram: "bg-violet-500/15 text-violet-400 border-violet-500/30",
};
    ...
    <div>
      ...
      <div className="p-5 space-y-5">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button className="text-xs px-3 py-1.5 rounded-xl bg-indigo-600/20 text-indigo-300 border border-indigo-500/30">All Platforms</button>
          {platforms.map((p) => (
            <button key={p} className={`text-xs px-3 py-1.5 rounded-xl border transition-colors text-gray-500 hover:${platColors[p]} glass glass-hover`}>{p}</button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {MOCK_TRENDS.map((trend) => (
            <div key={trend.id} className="glass glass-hover rounded-2xl p-5 group">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    ...
                    <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium border shrink-0 ${platColors[trend.platform]}`}>{trend.platform}</span>
                  </div>
                  ...
                  <div className="mb-2">
                    <div className="flex items-center justify-between text-xs mb-1">
                      ...
                      <span className={`font-semibold ${trend.relevance >= 90 ? "text-emerald-400" : trend.relevance >= 75 ? "text-indigo-400" : "text-gray-400"}`}>{trend.relevance}%</span>
                    </div>
                    <div className="h-1.5 bg-white/8 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full ${trend.relevance >= 90 ? "bg-emerald-500" : trend.relevance >= 75 ? "bg-indigo-500" : "bg-gray-500"}`}
                  ...
                  {trend.suggestedTemplate && (
                    <div className="flex items-center gap-2 mt-2">
                      <OpenClawBadge label="Suggested template" size="sm" />
