...
export default function WorldAtlasPage() {
  return (
    <div>
      <DashHeader title="World Atlas" description="Locked location profiles for consistent world-building"
        actions={
              ...
              <div key={loc.id} className="glass glass-hover rounded-2xl overflow-hidden group">
                ...
                <div className="h-28 bg-gradient-to-br from-indigo-900/40 via-slate-900/60 to-purple-900/30 relative flex items-end p-3">
                  <div className="absolute top-3 right-3 flex items-center gap-1.5">
                    ...
                    {loc.locked ? (
                      <span className="flex items-center gap-1 text-xs bg-indigo-500/15 text-indigo-300 border border-indigo-500/30 px-1.5 py-0.5 rounded-full">
                        <Lock className="w-2.5 h-2.5" /> Locked
                      </span>
                    ) : (
                      <button className="flex items-center gap-1 text-xs glass text-gray-500 px-1.5 py-0.5 rounded-full hover:text-white">
                        <Unlock className="w-2.5 h-2.5" /> Lock
                      </button>
                    )}
                  </div>
