...
export default function MultilingualEnginePage() {
  const sourceEpisode = MOCK_EPISODES[0]; // EP 1 – completed, source track ready
  ...
  const startAllPending = () => {
    ...
    setLanguages(prev =>
      prev.map(l => l.status === "Pending" ? { ...l, status: "Processing", progress: 0 } : l)
    );
    ...
    const interval = setInterval(() => {
      ...
      setLanguages(prev => {
        const next = prev.map(l =>
          l.status === "Processing" && (l.progress ?? 0) < 100
            ? { ...l, progress: Math.min(100, (l.progress ?? 0) + 8 + Math.floor(Math.random() * 5)) }
            : l
        );
        // Complete those at 100
        const resolved = next.map(l =>
          l.status === "Processing" && (l.progress ?? 0) >= 100
            ? { ...l, status: "Complete" as const, progress: 100 }
            : l
        );
        const allDone = resolved.every(l => l.status !== "Processing");
        if (allDone) {
  ...
  return (
    <div>
      <DashHeader title="Multilingual Engine" description={`AI dubbing and lip sync across ${languages.length}+ languages`} />
      <div className="p-5 space-y-5">
        ...
        <div className="glass rounded-2xl p-4">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            ...
            <button
              onClick={() => alert("Episode selector — coming in v1.1")}
              className="glass glass-hover text-gray-300 hover:text-white px-4 py-2 rounded-xl text-xs font-medium"
            >
              Change Episode
            </button>
          </div>
        ...
        <div className="glass rounded-2xl overflow-hidden">
          <div className="flex items-center justify-between px-5 py-3.5 border-b border-white/8">
            <div>
              ...
              {processingCount > 0 && (
                <p className="text-xs text-indigo-400 mt-0.5">{processingCount} processing…</p>
              )}
            </div>
            <button
              onClick={startAllPending}
              disabled={pendingCount === 0}
              className="text-xs bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 disabled:cursor-not-allowed text-white px-3 py-1.5 rounded-lg font-medium transition-all"
            >
              {pendingCount > 0 ? `Start ${pendingCount} pending` : "All started"}
            </button>
          </div>
                  ...
                  {lang.status === "Processing" && (
                    <div className="flex-1 max-w-xs">
                      <div className="h-1.5 bg-white/8 rounded-full overflow-hidden">
                        <div className="h-full bg-indigo-500 rounded-full transition-all" style={{ width: `${lang.progress}%` }} />
                      </div>
